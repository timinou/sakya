...
emacsclient -e '(prd-validate-all-cli)'

# Get dashboard metrics
emacsclient -e '(prd-dashboard-cli)'

# Quick one-liner status
emacsclient -e '(prd-quick-status)'

# Velocity report
emacsclient -e '(prd-velocity-report 7)'
#+END_SRC
...
Ensure the Emacs daemon is running for CLI access:
...
#+BEGIN_SRC bash
emacsclient -e '(prd-validate-file-cli "/path/to/file.org")'
emacsclient -e '(prd-validate-file-cli "/path/to/file.org" (quote json))'
#+END_SRC
...
#+BEGIN_SRC bash
emacsclient -e '(prd-validate-all-cli)'
emacsclient -e '(prd-validate-all-cli (quote json))'
#+END_SRC
...
#+BEGIN_SRC bash
emacsclient -e '(prd-validate-file-cli "{{file}}")'
#+END_SRC
...
FILE="$1"
RESULT=$(emacsclient -e "(prd-validate-file-cli \"$FILE\")" 2>/dev/null)
...
#+BEGIN_SRC bash
emacsclient -e '(prd-validate-all-cli)' | jq .
#+END_SRC
