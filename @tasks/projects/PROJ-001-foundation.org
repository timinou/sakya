...
** ITEM Install Rust dependencies
:PROPERTIES:
...
** ITEM Create global CSS theme architecture
:PROPERTIES:
...
** ITEM Create TypeScript type definitions
:PROPERTIES:
...
** ITEM Create Rust module structure and error types
:PROPERTIES:
...
** ITEM Create Tauri project commands
:PROPERTIES:
...
** ITEM Create Svelte 5 reactive state stores
:PROPERTIES:
...
** ITEM Create AppShell three-pane layout
:PROPERTIES:
...
*** Acceptance Criteria
- [ ] =AppShell.svelte=: CSS Grid with three columns (binder | editor | inspector)
- [ ] =PaneResizer.svelte=: Draggable divider with pointer events for resizing
- [ ] =Toolbar.svelte=: Project name display, view toggles, theme switcher
- [ ] =StatusBar.svelte=: Word count display, save status indicator
- [ ] =Binder.svelte=: Left sidebar shell (collapsible)
- [ ] =Inspector.svelte=: Right panel shell (collapsible)
- [ ] Binder and Inspector toggle via keyboard shortcuts and UI buttons
- [ ] Pane sizes persisted in UI store

** ITEM Create project launcher page
:PROPERTIES:
...
** ITEM Create Tauri service utilities
:PROPERTIES:
