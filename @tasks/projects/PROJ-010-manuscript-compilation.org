...
*** DONE Rust tests for compilation
:PROPERTIES:
:CUSTOM_ID: ITEM-102
:AGENT: [[file:../agents/rust-architect.org::#core][rust-architect:core]]
:EFFORT: 2h
...
**** Acceptance Criteria
- [X] Modal dialog component with overlay backdrop
- [X] Format selector: Markdown, HTML, Plain Text (radio group or segmented control)
- [X] Title page toggle (checkbox) with title and author text inputs
- [X] Chapter header style selector (dropdown: Numbered, Titled, Both, None)
- [X] Separator style selector (dropdown: Page Break, Three Stars, Horizontal Rule, Blank Lines)
- [X] Include synopsis toggle (checkbox)
- [X] Custom front matter textarea (collapsible section)
- [X] Preview pane showing first ~500 chars of compiled output, updating reactively on config changes
- [X] "Compile & Save" primary action button
- [X] "Cancel" secondary action button
- [X] Keyboard shortcut Cmd+Shift+E opens dialog from editor view
- [X] Escape closes dialog
- [X] Dialog styled consistently with existing Sakya modals (ConfirmDialog pattern)
