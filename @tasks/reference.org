...
│   └── planning.org       # Planning with checkpoints
├── elisp/
...
A checkpoint is a phase boundary heading placed within a category file, between groups of ITEMs. It marks the end of one phase and defines explicit gate conditions that must be satisfied before proceeding to the next.

** Purpose

Checkpoints formalize what was previously implicit: "these items form a phase, verify X before moving on." They provide:
- Clear phase boundaries within a flat ITEM list
- Verifiable criteria at each boundary
- Agent handoff points
- A structure for orchestration plans
...
:REVIEW_BY: agent-name(s) who review at this gate
:END:
...
| =REVIEW_BY= | No | Comma-separated names | Agent(s) who review at this gate |

** Naming Convention

=CHK-NNN-MM-slug= where:
- =NNN= — parent category number (e.g. =007= for PROJ-007)
- =MM= — sequential checkpoint number within that category (=01=, =02=, ...)
...
Checkpoints are *not* validated by the elisp system. The validator only processes:
- Headings with TODO keywords from =prd-todo-keywords= (ITEM, DOING, REVIEW, DONE, BLOCKED)
- Category headings matching =(PROJ|BUG|IMP)-[0-9]+=

=CHECKPOINT= headings match neither pattern — they are silently skipped. =CHK-*= IDs do not enter the item index. Metrics, dashboard, velocity, and burndown calculations are unaffected.

** Dependency Rules

- ITEMs should *NOT* use =DEPENDS= to reference =CHK-*= IDs — dependencies point to ITEMs only
- Items after a checkpoint should depend on the last item(s) before the checkpoint
- Checkpoints serve as visual and structural markers, not dependency targets

For detailed planning methodology including when and how to place checkpoints, see [[file:process/planning.org][the Planning process guide]].
...
IMPORTANT: Each ITEM MUST test its work independently to consider itself passing. This is also the case for visual tests; they MUST be passing as part of your implementation of the ITEM. If you are thinking of tests that span across several ITEMs (you should), those are e2e or integration tests that need to be done ON TOP of in-ITEM tests.
