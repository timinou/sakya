...
** DONE Add RecentProject model and Rust commands
...
*** Acceptance Criteria
- [X] RecentProject model with name, path, lastOpened fields
- [X] list_recent_projects reads, validates, prunes stale entries
- [X] add_recent_project upserts and caps at 10
- [X] remove_recent_project removes by path
- [X] Edge cases: missing file, corrupt JSON, stale paths
- [X] Commands registered in lib.rs

** DONE Add frontend store and launcher UI
CLOSED: [2026-02-15 Sat]
:PROPERTIES:
...
*** Acceptance Criteria
- [X] projectState has recentProjects array and load/remove methods
- [X] Launcher shows recent projects when list is non-empty
- [X] Each entry shows name, truncated path, click to open, X to remove
- [X] Opening a project adds it to recent list (via fire-and-forget add_recent_project)
- [X] Empty state (no recent projects) shows just Create/Open buttons
